...
/*
/**
 * Plugin Name: Responsive Welcome Bar
 * Plugin URI: https://zotabox.com/zbv2/promobar?utm_source=wordpress.com&utm_medium=pb%20app%20page&utm_campaign=ecommerce%20plugins&authuser=anonymous
 ...
 * Version: 1.0.7
 * Author: Zotabox
 * Author URI: https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=pb%20app%20page&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * License: SMB 1.0
 */

//Improve promo bar on mobile
...
function zb_pb_admin_menu() {
	add_menu_page('Responsive Welcome Bar', 'Welcome Bar', 'administrator', 'zb_pb', 'zb_pb_setting',plugins_url( 'zotabox.png', __FILE__ ));
}
...
function zb_pb_setting(){
	...
	if(isset($access_key) && !empty($access_key) && strlen($access_key) > 0 && $ztb_status_disconnect == $connected){
		...
		$button = '<a  target="zotabox" href="'.$domain_action.'/customer/access/PluginLogin/?customer='.$ztb_id.'&access='.$access_key.'&app=pb&platform=wordpress&utm_source='.str_replace('http://', '', home_url()) .'">
			Configure your tools
		</a>';
		$form = '';
	}else{
		$form = '<form class="ztb-register-form" target="_blank" method="POST" action="'.$domain_action.'/customer/access/PluginAuth?app=pb&utm_source='.str_replace('http://', '', home_url()) .'&utm_medium=PB%20wordpress%20plugin&utm_campaign=ecommerce%20plugins&platform=wordpress&access='.$access_key.'" id="account-info">
					<div class="form-group">
						...
						<input type="hidden" name="utm_medium" value="PB wordpress plugin" />
	...
	<div class="ztb-wrapper">
		<div class="ztb-logo">
			<a href="https://zotabox.com/dashboard/?utm_source='.str_replace('http://', '', get_bloginfo( 'home' )) .'&utm_medium=PB%20wordpress%20plugin&utm_campaign=ecommerce%20plugins&authuser=anonymous" title="Zotabox" target="zotabox">
				<img title="Zotabox" alt="Zotabox" src="'.plugins_url( 'assets/images/logo-zotabox.png', __FILE__ ).'">
			</a>
		</div>
		<div class="ztb-code-wrapper wrap">
			<div class="ztb-title">
				Responsive and Fully Customizable Bar. Promote Free Shipping, Free Gifts, Coupons, +10 more popular website marketing tools included.
			</div>
...
function update_zb_pb_code(){
	...
	if(!isset($domain) || empty($domain)){
		header("Location: ".admin_url()."admin.php?page=zb_pb");
	}else{
