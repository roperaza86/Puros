...
class SYN_Shipping_UPS extends SYN_Shipping_Method {
	...
	public function get_shipping_request( $package ) {
		...
		foreach( $this->addresses as $address_key => $origin_address ){
			...
			$this->set_package_requests( $package );
	...
	private function get_estimate( $code, $response ){
		...
		if ( $xml->Response->ResponseStatusCode == 1 ) {
			...
			$service_name = $this->custom_methods[ $code ][ 'name' ];
			...
			$rate_name = ( empty( $response[ 'title' ] ) ? $this->title . ( empty( $this->title ) ? '' : ' ' ) : $response[ 'title' ] . ' ' ) . $service_name;
	...
	private function check_generic_package( $xml, $origin_address, $address ){
    ...
    private function set_package_requests( $package ) {
    	...
    	switch ( $this->packing_method ) {
    		case 'per_item' :
	    	default :
	    		$this->per_item_shipping( $package );
				break;
	    	case 'weight' :
	    		$this->weight_only_shipping( $package );
				break;
	    	case 'box_packing':
	    		$this->box_shipping( $package );
	    		break;
    	}
    ...
    private function per_item_shipping( $package ) {
    	...
    	foreach ( $package['contents'] as $item_id => $values ) {
    		...
    		$ups_method_restriction = get_post_meta( $values[ 'product_id' ], '_ups_method_restriction', true );
			...
			for ( $i = 0; $i < $values[ 'quantity' ] ; $i++){
				...
				if( $ups_method_restriction ){
					...
					if( !isset( $this->packages[ $ups_method_restriction ] ) )
						$this->packages[ $ups_method_restriction ] = null;
				
					$this->packages[ $ups_method_restriction ] .= $piece;
